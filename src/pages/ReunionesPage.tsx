...
import type { ActaVisita, ActaVisitaRow, OrganizacionReunion, TipoReunion } from '../types/actas'
...
import {
  ...
  updateActaPdf,
  updateActaEstado,
  updateActaAsistencia,
} from '../lib/actasService'
...
const TIPO_REUNION_LABELS: Record<TipoReunion, string> = {
...
function estadoChipClass(estado: string): 'info' | 'warning' | 'success' {
...
function orgLabel(acta: ActaVisitaRow): string {
  const org = acta.organizacion as OrganizacionReunion | undefined
  return org?.nombre ?? acta.establecimiento_nombre ?? '—'
}

function getReunionIdFromHash() {
  const [, query = ''] = window.location.hash.split('?')
  return new URLSearchParams(query).get('reunionId')
}

function buildReunionHash(reunionId?: string | null) {
  const params = new URLSearchParams()
  if (reunionId) params.set('reunionId', reunionId)
  const query = params.toString()
  return query ? `/reuniones?${query}` : '/reuniones'
}
...
export function ReunionesPage() {
  ...
  const [reuniones, setReuniones] = useState<ActaVisitaRow[]>([])
  ...
  useEffect(() => {
    const syncFromHash = () => {
      const id = getReunionIdFromHash()
      if (!id) { setDetailActa(null); return }
      const match = reuniones.find((r) => r.id === id)
      if (match) setDetailActa(match)
    }
    syncFromHash()
  ...
  const openDetail = useCallback((acta: ActaVisitaRow) => {
    setDetailActa(acta)
    window.location.hash = buildReunionHash(acta.id)
  }, [])

  const closeDetail = useCallback(() => {
    setDetailActa(null)
    if (getReunionIdFromHash()) window.location.hash = buildReunionHash()
  }, [])
  ...
  async function handleFormSubmit(
    ...
    if (insertErr || !id) {
      setSubmitMsg({ type: 'error', text: insertErr ?? 'Error al guardar la reunión.' })
      setSubmitting(false)
      return
    }
    ...
    if (asistenciaFile) {
      ...
      if (!asistErr && path && url) await updateActaAsistencia(id, path, url)
    }
    ...
    if (!pdfOk) {
      ...
      setSubmitMsg({
        type: 'error',
        text: `Reunión guardada, pero el PDF falló: ${pdfErrorMsg} Puedes reintentar desde el detalle.`,
      })
    } else {
      setSubmitMsg({ type: 'success', text: 'Reunión guardada y PDF generado correctamente.' })
    }
      ...
      <div className="space-y-6">
        <Card tone="strong" className="p-5 sm:p-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-xs font-semibold uppercase tracking-[0.24em] text-[#0057B8]">Reuniones</p>
              <h3 className="text-xl font-light text-slate-800">Nueva acta de reunión</h3>
            </div>
          </div>
        </Card>

        {submitMsg && (
          <Alert tone={submitMsg.type === 'success' ? 'success' : 'danger'}>{submitMsg.text}</Alert>
        )}
    ...
    <div className="space-y-6">
      <Card tone="strong" className="p-6 sm:p-7">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            <p className="text-xs font-semibold uppercase tracking-[0.24em] text-[#0057B8]">Reuniones</p>
            <h3 className="mt-2 text-3xl font-light text-slate-800">Actas de reunión</h3>
            <p className="mt-3 max-w-2xl text-sm leading-7 text-slate-500">
              Registro de reuniones con establecimientos, organismos externos, empresas y equipos de trabajo.
            </p>
          </div>
      ...
      {submitMsg && (
        <Alert tone={submitMsg.type === 'success' ? 'success' : 'danger'}>
          ...
          <Button
            ...
            variant="ghost"
            size="sm"
            className="ml-3 text-xs underline opacity-70 hover:bg-transparent hover:opacity-100"
      ...
      <Card tone="surface" padding="none" className="overflow-hidden">
        <div className="flex flex-col gap-3 border-b border-slate-200 px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <h4 className="mt-0.5 text-lg font-light text-slate-800">Reuniones registradas</h4>
          </div>
          {!loading && reuniones.length > 0 && (
            <p className="text-xs text-slate-400">
              {reuniones.length} registro{reuniones.length !== 1 ? 's' : ''}
            </p>
          )}
        </div>
        ...
        <DataTable responsive="md">
          <DataTableHead>
            <DataTableRow>
              ...
              <DataTableHeaderCell>Organización / Contraparte</DataTableHeaderCell>
            ...
            {loading ? (
              ...
              <DataTableRow>
                <DataTableCell colSpan={9} className="px-5 py-12 text-center text-sm text-red-600">
                  Error al cargar reuniones: {loadError}
                </DataTableCell>
              </DataTableRow>
            ) : reuniones.length === 0 ? (
              <DataTableRow>
                <DataTableCell colSpan={9} className="px-5 py-14 text-center text-slate-400">
                  <p className="text-sm font-medium">No hay reuniones registradas</p>
                  <p className="mt-1 text-xs">Usa &ldquo;Nueva reunión&rdquo; para comenzar el registro.</p>
                </DataTableCell>
              </DataTableRow>
            ) : (
              reuniones.map((acta) => {
                ...
                return (
                  <DataTableRow key={acta.id} className="group transition-colors hover:bg-blue-50/30">
                    <DataTableCell className="whitespace-nowrap font-mono text-xs font-semibold text-[#0033A0]">
                      {acta.folio}
                    </DataTableCell>
                    <DataTableCell className="whitespace-nowrap text-slate-700">
                      {formatFecha(acta.fecha_visita)}
                    </DataTableCell>
                    <DataTableCell>
                      <p className="font-medium leading-tight text-slate-800">{orgLabel(acta)}</p>
                    ...
                    <DataTableCell>
                      <StatusBadge tone="info" className="text-xs">
                        {org ? (TIPO_REUNION_LABELS[org.tipo] ?? org.tipo) : '—'}
                      </StatusBadge>
                    </DataTableCell>
                    <DataTableCell className="whitespace-nowrap text-xs text-slate-500">
                      {acta.hora_inicio} – {acta.hora_termino}
                    </DataTableCell>
                    <DataTableCell className="text-center text-slate-600">
                      {acta.participantes.length}
                    </DataTableCell>
                    <DataTableCell className="text-center text-slate-600">
                      {acta.acuerdos.length}
                    </DataTableCell>
                    <DataTableCell>
                      <StatusBadge tone={estadoChipClass(acta.estado ?? 'Registrada')}>
      ...
      {detailActa && (
        <ActaDetailModal
          ...
          onUpdated={(updated) => {
            ...
            window.location.hash = buildReunionHash(updated.id)
          }}
        />
      )}
    </div>
